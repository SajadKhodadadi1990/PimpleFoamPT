...
//                 ibCell = ownCell;
        ...
        deleteDemandDrivenData(ibFacesInPtr_);
...
//---------------------------------------------------------------------
void Foam::immersedBoundaryFvPatch::makeIbFacesIn() const
{
			//Info<< "call ****makeIbFacesIn**** in /home/user/Desktop/IB/immersedBoundarySrc/immersedBoundary/immersedBoundaryFvPatch. "<<endl;
    ...
    const labelList& ibc = ibCells();
    const volScalarField& gammaExtI = gammaExt();
    const scalarField& gammaExtIn = gammaExtI.internalField();
    
    const surfaceScalarField& sgammaI = sGamma(); 
   
    dynamicLabelList ibFIn(2*ibc.size());
    const unallocLabelList& owner = mesh_.owner();
    const unallocLabelList& neighbour = mesh_.neighbour();
    scalar X=0;
    forAll(sgammaI,faceI)
    {
		if(gammaExtIn[owner[faceI]] == 0
        || gammaExtIn[neighbour[faceI]] == 0)
		{
			if(gammaExtIn[owner[faceI]] == 0
			&& gammaExtIn[neighbour[faceI]] == 0)
			{
    ...
    ibFacesInPtr_ = new labelList(ibFIn.xfer());
}
...
const Foam::labelList& Foam::immersedBoundaryFvPatch::ibFacesIn() const
{
//Info<< "call ****ibFacesIn**** in /home/user/Desktop/IB/immersedBoundarySrc/immersedBoundary/immersedBoundaryFvPatch. "<<endl;
