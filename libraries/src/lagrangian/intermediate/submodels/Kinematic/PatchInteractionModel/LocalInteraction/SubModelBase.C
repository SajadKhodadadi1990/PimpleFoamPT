...
template<class CloudType>
Foam::SubModelBase<CloudType>::SubModelBase(CloudType& owners)
:
    owners_(owners),
...
:
    ...
    modelName_(word::null),
    coeffDicts_(dict.subDict(modelType + dictExt))
{}
...
template<class CloudType>
const CloudType& Foam::SubModelBase<CloudType>::owners() const
{
    return owners_;
}
...
template<class CloudType>
const Foam::dictionary& Foam::SubModelBase<CloudType>::coeffDicts() const
{
    return coeffDicts_;
}
...
{
    bool def = coeffDicts_.lookupOrDefault<bool>("defaultCoeffs", false);
    if (printMsg && def)
    {
        Info<< incrIndent;
...
template<class CloudType>
CloudType& Foam::SubModelBase<CloudType>::owners()
{
    return owners_;
}
...
bool Foam::SubModelBase<CloudType>::outputTime() const
{
    return
        active()
     //&& owners_.solution().transient()
     && owners_.db().time().outputTime();
}
...
) const
{
    ...
    const dictionary& properties = this->owners().outputProperties();
...
{
    dictionary& properties = this->owners().outputProperties();
...
{
    os.writeKeyword("owners") << owners_.name() << token::END_STATEMENT
        << nl;
